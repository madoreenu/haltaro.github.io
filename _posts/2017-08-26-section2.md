...
* 線形モデルにおける訓練データの当てはめには，**最小二乗法**が用いられる．最小二乗法では，残渣二乗和$$\mathrm{RSS}(\beta)=\left(\mathbf{y} - \mathbf{X} \beta \right)^{T}\left(\mathbf{y} - \mathbf{X} \beta \right)^{T}$$を最小化する$$\beta$$を求める．
